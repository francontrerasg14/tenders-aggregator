...
def parse_args():
    ...
    ap.add_argument("--when", choices=["updated","published","either"], default="either")
    ap.add_argument("--cpv", nargs="+", default=["09330000","45261215","45315300"])
    ap.add_argument("--cpv-mode", choices=["exact","prefix"], default="exact")
    ap.add_argument("--cpv-scope", choices=["folder","lots","both"], default="folder")
    ap.add_argument("--config", default="config.yml")
    ap.add_argument("--out", default=None)
    ap.add_argument("--no-placsp", action="store_true")
...
def main():
    ...
    if not args.no_rss and os.path.exists(args.config):
        ...
        for src in (conf.get("rss_sources") or []):
            ...
            follow_detail = bool(src.get("follow_detail", True))  # por defecto True
            delay = float(src.get("polite_delay", 1.2))
    ...
    cols = ["fuente","expediente","objeto","organo","estado","importe","cpv","fecha_published","fecha_updated","enlace",
            "cpv_source","importe_source","detail_status"]
    with open(out,"w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f, fieldnames=cols); w.writeheader()
        for r in uniq: w.writerow({c: r.get(c,"") for c in cols})
    print(f"[ok] CSV: {out} | filas={len(uniq)}")
